...
public class BetterBrowser implements Browser
{
    ...
    public static String reversePathURL( String urlString )
    {
        ...
        return encodeSpace( join( list, "." ) +
                            urlString.substring( urlString.indexOf( url.getHost() ) + url.getHost().length(), hIndex ) )
            .toLowerCase();
    }
    ...
    @Override public boolean loadPage( String url )
    {
    ...
    @Override public List<String> getURLs()
    {
        ...
        for ( Element e : doc.select( "a[href]" ) )
        {
            try
            {
                if ( new URL( e.attr( "abs:href" ) ).getProtocol().startsWith( "http" ) )
                {
