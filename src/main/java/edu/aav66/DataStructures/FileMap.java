...
public class FileMap extends ArrayMap
{
    ...
    static String newFileName() { return fileCounter++ + ".txt"; }
    ...
    public String get( Object keyAsObject )
    {
        ...
        if ( index < 0 )
            return null;
        // Get that file and return the key.
        return getFile( index ).get( key ); // FIXED
        ///
    }
    ...
    void join( int index )
    {
        ...
        left.write();
        new File( right.fileName ).delete();
        ///
    }

    public String remove( Object keyAsObject )
    {
        // Set to false to enable removal logic
        if ( false )
            return null;
        ...
        if ( file.size < entries.length / 2 )
        {
            ...
            if ( index < size - 1 )
            {
                ...
                // If the map at index at or greater than capacity, split it
            ...
            else if ( index > 0 )
            {
                ...
                // After join, check if the resulting file is over capacity and split if necessary
    ...
    protected class Iter implements Iterator<Map.Entry<String, String>>
    {
        ...
        public boolean hasNext() { return index < size && iter.hasNext(); }
